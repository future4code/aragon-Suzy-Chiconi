...
export class UserController {
  public signup = async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (nickname.length < 3 || password.length < 6) {
        errorCode = 422
        throw new Error("Nickname must be longer then 3 characters and password mut be longer then 6 characters.")
      }
  ...
  public getAllUsers = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const userByName = await userDatabase.getUserByNickname(search)
