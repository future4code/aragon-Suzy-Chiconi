...
/*
## Exercício 1
Crie um arquivo chamado data.ts que contenha e exporte o array de usuários:

Dicas:
    - Cada usuário deve ser representado por um type User, com as propriedades definidas 
    acima.
    - A propriedade ‘role’ deve ser um enum USER_ROLE com os seguintes valores possíveis: 
    “ADMIN” ou “NORMAL”.
    */
...
/*
## Exercício 2
Crie um endpoint de busca de usuários da lista. Este endpoint deve permitir a busca de 
usuários pela propriedade ‘role’. Caso nenhum valor de ‘role’ seja definido, retornamos 
...
Dicas:
    - ‘role’ é uma query opcional. 
    Caso seja fornecida verifique se ela vale “NORMAL” ou “ADMIN”.
*/
...
Validação de Input:
    ...
    - name e email devem ser do tipo string, age deve ser do tipo number, enquanto 
      role só pode assumir dois valores: “ADMIN” ou “NORMAL”.
...
Saídas possíveis:
    - Cada erro deve retornar o seu respectivo status code e uma mensagem descrevendo 
      a situação.
    - Para sucesso, deve retornar o status de criação 201, mensagem de sucesso e a 
      lista de usuários atualizada.

Dicas:
    - Para não se preocupar em contar o tamanho da lista ao criar uma id automatizada, 
      utilize o Date.now()
...
app.post("/users", (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!name || !email || !age || !role) {
            errorCode = 422
            throw new Error("Error: Product name, email, age or role is required")
        }

        if (typeof name !== "string" || typeof email !== "string" || typeof age !== "number") {
            errorCode = 422
            throw new Error("Error: name and email must be a string, age must be a number or role must be ADMIN or NORMAL")
...
/*
## Exercício 4
Crie um endpoint que edita o e-mail de um determinado usuário e retorna o usuário 
atualizado.
...
Regras de negócio:
    - Se o id informado for válido, mas não existir em um dos usuários, um erro deverá 
      ser exibido.
    - Caso o e-mail enviado já exista para outro usuário cadastrado, um erro deverá ser 
      exibido.
...
app.put("/users/:id", (req: Request, res: Response) => {
    ...
    try {
        ...
        if (userIndex < 0) {
            errorCode = 409
            throw new Error("Error: Id doesn't match a valid user.")
        }
